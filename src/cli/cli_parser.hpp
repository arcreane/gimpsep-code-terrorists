...
inline ParsedArguments parse_arguments(int argc, char **argv)
{
    ...
    try
    {
        ...
        options.add_options()("h,help", "Display this help message")("op,operation", "The operation to perform (dilate, erode, resize, brightness, stitch, canny, video-gray, detect-faces, bg-subtract, detect-objects, inpaint)", cxxopts::value<std::string>())("i,input", "Input image/video file path(s). Multiple allowed for stitch.", cxxopts::value<std::vector<std::string>>())("o,output", "Output image/video file path", cxxopts::value<std::string>())
            // Core operation-specific options
            ("k,kernel_size", "Kernel size for dilation/erosion (positive odd integer)", cxxopts::value<int>()->default_value("3"))("f,factor", "Resize factor (e.g., 1.5 for 150%, 0.5 for 50%)", cxxopts::value<double>())("b,brightness", "Value to add/subtract for brightness adjustment (-255 to 255)", cxxopts::value<int>()->default_value("0"))("t1,threshold1", "First threshold for the Canny edge detector hysteresis procedure", cxxopts::value<double>()->default_value("100.0"))("t2,threshold2", "Second threshold for the Canny edge detector hysteresis procedure", cxxopts::value<double>()->default_value("200.0"))
            ...
            ("yolo_cfg", "Path to YOLO .cfg file (for detect-objects)", cxxopts::value<std::string>())("yolo_weights", "Path to YOLO .weights file (for detect-objects)", cxxopts::value<std::string>())("yolo_names", "Path to YOLO .names file (for detect-objects)", cxxopts::value<std::string>())("conf", "Confidence threshold (for detect-objects)", cxxopts::value<float>()->default_value("0.5"))("nms", "NMS threshold (for detect-objects)", cxxopts::value<float>()->default_value("0.4"))
            // Inpainting options
            ("m,mask", "Path to the mask image (for inpaint)", cxxopts::value<std::string>())("radius", "Inpainting radius (for inpaint)", cxxopts::value<double>()->default_value("3.0"))("inpaint_method", "Inpainting method: NS or TELEA (for inpaint)", cxxopts::value<std::string>()->default_value("NS"));
        ...
        if ((args.operation == "video-gray" || args.operation == "bg-subtract") && args.input_files.size() != 1)
        {
